...
### 4.1 Read the budget (rich UI)
```http
GET /events/evt_123/budget      → BudgetItemView[]
```
```json
[
...
```
- `source` (`ProvenanceSource`): `organizer | ai_from_chat | ai_from_document | …`. A line read off a photo is `ai_from_document` and carries `sourceDocumentId` back to its document.
- `version` is an optimistic-lock counter — pass `expectedVersion` on update to avoid clobbering a concurrent edit.
- For **per-item coverage/funding status** (FUNDED/PARTIALLY_FUNDED/UNFUNDED + covered/remaining), use `GET /events/:id/report` or the AI `get_budget` tool — plain `/budget` returns the planned lines, not their allocation coverage.

### 4.2 Add/update a line — direct REST
```http
POST  /events/evt_123/budget/items        { "name": "Photography", "targetValue": "2000000" }
PATCH /events/evt_123/budget/items/bi_2    { "targetValue": "9000000", "isPublic": true, "expectedVersion": 2 }
```
Returns the updated `BudgetItemView`.

### 4.3 Budget via the AI (staged)
```http
POST /assistant   { "message": "add catering at 5m", "conversationId": "conv_1" }
```
```json
{
  "conversationId": "conv_1", "activeEventId": "evt_123",
  "reply": "Add budget item \"Catering\" at 5000000? Confirm and I'll add it.",
  "staged": ["pc_bud_1"]
}
```
→ `GET /events/evt_123/pending` to render the card → `POST /events/evt_123/pending/pc_bud_1/confirm` → `ExecutionResult { message: "Added budget item \"Catering\" (5000000).", data: <BudgetItemView> }`.

### 4.4 The budget spreadsheet viewer + "mark as covered" (new)

For a panel that lists every budget item with a per-item coverage column: use `GET /events/:id/report` (not plain `/budget` — see §4.1), whose `budget.items[]` already carries the coverage state, no extra call needed:

```json
{
  "budget": {
    "total": "15030000", "covered": "5000000", "remaining": "10030000",
    "items": [
      { "name": "Catering", "target": "5000000", "covered": "5000000", "remaining": "0", "status": "FUNDED" },
...
**"Covered" is derived from real money, never a settable flag** — same rule as every other status in this app (pledge status, fulfillment status). There is no `PATCH .../covered` endpoint and there won't be one; a bare checkbox would let the sheet show funding that never happened. Instead, "mark as covered" means **earmarking a real payment against that line** — `status` flips to `FUNDED`/`PARTIALLY_FUNDED` automatically once enough is allocated, and that's the exact same live number the **AI** and the **public page** (§3) both read — nothing further to sync.

**Manual flow (2 calls, both already exist):**
```http
...
```
If the money was already recorded as a payment against an existing pledge (not a fresh direct contribution), skip the first call and use that payment's `fulfillmentId` directly — you can get it from `GET /events/:id/pledges` (§ contributions/pledges panel doc), each split's `id`.

**Via AI chat (new):** the organizer can now say things like *"allocate John's payment to catering"* or *"put 200k of Sarah's money toward the venue"* — the AI resolves the person's most recent payment with unallocated value left, stages the same allocation, and the confirm flow is identical to every other staged action (§ main contract). If they have no unallocated payment, or ask for more than what's left, the AI asks instead of guessing.
